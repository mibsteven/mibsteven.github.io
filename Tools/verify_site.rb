...
archive_sections.each do |archive|
  ...
  assert(dates.length == 10, "each language archive must contain ten dated entries")
  assert(dates == dates.sort.reverse, "archive entries must be reverse chronological")
end

assert(index.scan(/class="panel app-card"/).length == 48, "homepage must retain 24 apps per language")
...
assert(app_icons.length == 58, "homepage app icon count changed unexpectedly")
...
assert(thumbnail_files.length == 24, "expected 24 optimized app thumbnails")
