...
archive_sections.each do |archive|
  ...
  assert(dates.length == 13, "each language archive must contain thirteen dated entries")
  assert(dates == dates.sort.reverse, "archive entries must be reverse chronological")
end

assert(index.scan(/class="panel app-card"/).length == 50, "homepage must retain 25 apps per language")
...
assert(app_icons.length == 60, "homepage app icon count changed unexpectedly")
...
assert(thumbnail_files.length == 25, "expected 25 optimized app thumbnails")
